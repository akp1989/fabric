...
function registerEnrollUserCredentials() {
  IDNAME=user1
  IDTYPE=client

  CSR_CNAME=\'$IDNAME.$ORGANIZATION_NAME.$DOMAIN_NAME\'
  CSR_CNAME_TMP=$IDNAME.$ORGANIZATION_NAME.$DOMAIN_NAME
  ./client/fabric-ca-client register -d --id.name $CSR_CNAME_TMP --id.secret clientroot --id.type $IDTYPE -u https://$SERVER_ADDRESS:$SERVER_PORT --csr.cn $CSR_CNAME --csr.names $CSR_NAME --csr.hosts $CSR_HOSTS
  printf  "\n\n\n-------> The user credentials for $IDNAME was successfully registered for $ORGANIZATION_NAME.$DOMAIN_NAME \n\n\n"
 
  export FABRIC_CA_CLIENT_HOME=$PWD/client/profiles/$IDNAME-$ORGANIZATION_NAME
  export FABRIC_CA_CLIENT_MSPDIR=$PWD/client/profiles/$IDNAME-$ORGANIZATION_NAME/msp
  export FABRIC_CA_CLIENT_TLS_CERTFILES=$PWD/client/tls-certs/ca-cert.pem
  ./client/fabric-ca-client enroll -d -u https://$CSR_CNAME_TMP:clientroot@$SERVER_ADDRESS:$SERVER_PORT --id.type $IDTYPE --csr.cn $CSR_CNAME --csr.names $CSR_NAME --csr.hosts $CSR_HOSTS
  printf  "\n\n\n-------> The user credentials for $IDNAME was successfully enrolled for $ORGANIZATION_NAME.$DOMAIN_NAME \n\n\n"
}
