function enrollCAAdmin() {
  ...
  ./client/fabric-ca-client enroll -d -u https://icadmin.boot:icadminpw@$SERVER_ADDRESS:$SERVER_PORT --csr.cn $CSR_CNAME --csr.names $CSR_NAME --csr.hosts $CSR_HOSTS 
...
function registerEnrollTLSCredentials() {
  if [ "$MODE" == "peer" ]; then
    IDNAME=tls-peer0
	IDTYPE=peer
    CSR_CNAME=\'$IDNAME.$ORGANIZATION_NAME.$DOMAIN_NAME\'
...
function populateAdminUser() {
  ...
  mkdir $ADMIN_USER_MSP/msp/intermediatecerts
  ...
  cp $PWD/client/tls-certs/cacerts/ca-cert.pem $ADMIN_USER_MSP/msp/cacerts/ca-cert.pem
  cp $PWD/client/tls-certs/intermediatecerts/ca-cert.pem $ADMIN_USER_MSP/msp/intermediatecerts/ca-cert.pem
...
function populateTLSUser() {
  ...
  mkdir $TLS_USER_MSP/intermediatecerts
  ...
  cp $PWD/client/tls-certs/cacerts/ca-cert.pem $TLS_USER_MSP/cacerts/ca-cert.pem
  cp $PWD/client/tls-certs/intermediatecerts/ca-cert.pem $TLS_USER_MSP/intermediatecerts/ca-cert.pem
...
function populatePeerUser() {
  if [ "$MODE" == "peer" ]; then
    ...
    echo "Invalid ID type to populate"
  fi
  ...
  mkdir $PEER_USER_MSP/intermediatecerts
 ...
 #cp $PWD/client/tls-certs/ca-cert.pem $PEER_USER_MSP/cacerts/ca-cert.pem
  cp $PWD/client/tls-certs/cacerts/ca-cert.pem $PEER_USER_MSP/cacerts/ca-cert.pem
  cp $PWD/client/tls-certs/intermediatecerts/ca-cert.pem $PEER_USER_MSP/intermediatecerts/ca-cert.pem
...
function populatePeerMSP() {
  ...
  mkdir $PEER_MSP/intermediatecerts
  ...
  cp $PWD/client/tls-certs/cacerts/ca-cert.pem $PEER_MSP/cacerts/ca-cert.pem
  cp $PWD/client/tls-certs/intermediatecerts/ca-cert.pem $PEER_MSP/intermediatecerts/ca-cert.pem
...
function populateOrdererMSP() {
  ...
  mkdir $ORDERER_MSP/intermediatecerts
  ...
  cp $PWD/client/tls-certs/cacerts/ca-cert.pem $ORDERER_MSP/cacerts/ca-cert.pem
  cp $PWD/client/tls-certs/intermediatecerts/ca-cert.pem $ORDERER_MSP/intermediatecerts/ca-cert.pem
...
CSR_NAME='C=US,ST=California,L=SanFrancisco,O=ripe,OU=product'

#enrollCAAdmin
#registerPeerCredentials
#enrollPeerCredentials
#enrollTLSAdmin
#registerEnrollTLSCredentials
